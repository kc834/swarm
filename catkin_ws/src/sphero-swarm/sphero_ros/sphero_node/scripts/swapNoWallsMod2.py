    ...
    while (dist2goalAll > 0.1*(Num+2)): # not at goal point
        ...
        for i in range(Num):
            ...
            for j in range(numAll):
                ...
                # update time
                timeList[0][j] = timeList[1][j]
                timeList[1][j] = time.time()
                dt = timeList[1][j] - timeList[0][j]
            ...
            for j in range(numAll):
                ...
                if j!=i+Diff:
                    ...
                    if numpy.linalg.norm(statejTmp-stateiTmp) <= obj.Dneighbor[i+Diff]:
                        ...
                        xDiff = poseAll[0,j] - poseAll[0,i+Diff]
                        yDiff = poseAll[1,j] - poseAll[1,i+Diff]
                        ...
                        # If this sphero's waypoint is mad close to the other sphero
                        if (DistColl <= obj.Ds):

                            # scale factor
                            sF = obj.Ds/Di
                            # Change waypoint of the sphero
                            loc_goal[j][0] = loc_goal[j][0] - xDiff * sF
                            loc_goal[j][1] = loc_goal[j][1] - yDiff * sF
                            # loc_goal[i][0] = sF * poseAll[0,i+Diff] + (1 - sF) * poseAll[0,j]
                            # loc_goal[i][1] = sF * poseAll[1,i+Diff] + (1 - sF) * poseAll[1,j]
