...
if __name__=="__main__":
    ...
    rate = rospy.Rate(16) # set publish rate
    ...
    for i in range(Num):
        ...
        vel_msg = geometry_msgs.msg.Twist(geometry_msgs.msg.Vector3(80,0,0), \
                    ...
                    if dist2goal[i] > 0.1:
                        ...
                        try:
                            ...
                            if Region != 5:
                                ...
                                v_x_V_new = moveVect[i] * vel_mag * math.cos(thetaNew)
                                v_y_V_new = moveVect[i] * vel_mag * math.sin(thetaNew)
                    	       ...
                    	       # convert global frame to sphero frame
                                ...
                                v_x_V_new = 0.8 * moveVect[i] * vel_mag * math.cos(thetaNew)
                                v_y_V_new = 0.8 * moveVect[i] * vel_mag * math.sin(thetaNew)
                                print v_x_V_new
                                print v_y_V_new
                                print "run"
                                v_vect_V_update = numpy.array([[v_x_V_new],[v_y_V_new]])
                        ...
                        vel_msg_zero = geometry_msgs.msg.Twist(geometry_msgs.msg.Vector3(0,0,0), \
                                                      	geometry_msgs.msg.Vector3(0,0,0))
                        mag = numpy.linalg.norm([v_x_S, v_y_S])
                        print time.time() - delay[i]
                        if time.time() - delay[i] > 0.5 and (v_x_S != 0 or v_y_S != 0):
                            pub[i].publish(vel_msg)
