...
if __name__=="__main__":
    ...
    vel_msg_2 = geometry_msgs.msg.Twist(geometry_msgs.msg.Vector3(0,100,0), \
                                      geometry_msgs.msg.Vector3(0,0,0))

    vel_msg_3= geometry_msgs.msg.Twist(geometry_msgs.msg.Vector3(-100,0,0), \
                                      geometry_msgs.msg.Vector3(0,0,0))

    vel_msg_4 = geometry_msgs.msg.Twist(geometry_msgs.msg.Vector3(0,-100,0), \
                                      geometry_msgs.msg.Vector3(0,0,0))
    ...
    rospy.loginfo("Velocity: {0}".format(vel_msg_2))
    start = time.time()
    while time.time()-start < 2: # not rospy.is_shutdown():
        pub.publish(vel_msg_2)
        rate.sleep()

    rospy.loginfo("Init Odometry: {0}".format(init_odom))
    rospy.loginfo("Last Odometry: {0}".format(odometry))

    rospy.loginfo("Velocity: {0}".format(vel_msg_3))
    start = time.time()
    while time.time()-start < 2: # not rospy.is_shutdown():
        pub.publish(vel_msg_3)
        rate.sleep()
    ...
    rospy.loginfo("Velocity: {0}".format(vel_msg_4))
    start = time.time()
    while time.time()-start < 2: # not rospy.is_shutdown():
        pub.publish(vel_msg_4)
        rate.sleep()
